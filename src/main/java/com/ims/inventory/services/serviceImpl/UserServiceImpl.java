...
public class UserServiceImpl implements UserService {
    ...
    public ResponseDto login(User user) {
        OauthResponse responseDto = new OauthResponse();
        ...
        if (user != null && user.getUserName() != null && user.getUserPassword() != null) {
            ...
            if (userOptional.isPresent()) {
                if (userOptional.get().getStatus().equals(ActiveStatus.ACTIVE))  {
                    ...
                    String body = restTemplate.exchange(environment.getProperty("auth-server.url")+"grant_type=password&username="+user.getUserName()+"&password="+user.getUserPassword(), HttpMethod.POST, headersHttpEntity,String.class).getBody();
                    Gson gson = new Gson();
                    responseDto = gson.fromJson(body, OauthResponse.class);
    ...
    @Override
    public ResponseDto loginSuperAdmin(SuperAdmin superAdmin) {
        ResponseDto responseDto = new ResponseDto();

        if (superAdmin.getUserName() != null && superAdmin.getPassword() != null) {
            QSuperAdmin qSuperAdmin = QSuperAdmin.superAdmin;
            BooleanBuilder booleanBuilder = new BooleanBuilder();
            booleanBuilder.and(qSuperAdmin.userName.eq(superAdmin.getUserName()));
            booleanBuilder.and(qSuperAdmin.password.eq(superAdmin.getPassword()));

            Optional<SuperAdmin> superAdminOptional = superAdminRepository.findOne(booleanBuilder);
